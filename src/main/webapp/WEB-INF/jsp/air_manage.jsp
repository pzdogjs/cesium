	...
	<div id="win" class="easyui-window" title="客户数据编辑" style="width:500px;height:300px"   
	    ...
	    <form id="editForm" method="post">
	    	<%--提供id隐藏域 --%>
				...
				start：<input type="text" name="start" class="easyui-validatebox" data-options="required:true"/><br/>
		  		end：<input type="text" name="end" class="easyui-validatebox" data-options="required:true"/><br/>
				...
				type：<input type="text" name="type" class="easyui-validatebox" data-options="required:true"/><br/>
				...
				h：<input type="text" name="h" class="easyui-validatebox" data-options="required:true"/><br/>
  	...
  	<script type="text/javascript">
  		$(function(){
  			$("#list").datagrid({
  				//url:后台数据查询的地址
  				url:"../airController/show_all.action",
  				//columns：填充的列数据
  					//field:后台对象的属性
  					//tille:列标题
  				columns:[[
  			...
  			$("#editBtn").click(function(){
  				...
  				//表单回显
  				$("#editForm").form("load","../airController/findById.action?id="+rows[0].id);
  			...
  			$("#deleteBtn").click(function(){
  				...
  				$.messager.confirm("提示","确认删除数据吗?",function(value){
  					if(value){
  						...
  						//传递到后台
  						$.post("../airController/del.action",idStr,function(data){
  							if(data.success){
  	  							//刷新datagrid
  	  							$("#list").datagrid("reload");
